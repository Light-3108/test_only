...
if __name__ == '__main__':
    ...
    agent_1 = make_agent(name='agent', env=venv, args=args, device=device)
    agent_2 = make_agent(name='agent', env=venv, args=args, device=device)
    # adversary_agent, adversary_env = None, None
    if is_paired or args.use_accel_paired:
        adversary_agent_1 = make_agent(name='adversary_agent', env=venv, args=args, device=device)
        adversary_agent_2 = make_agent(name='adversary_agent', env=venv, args=args, device=device)
    if is_training_env:
    ...
    if args.checkpoint and os.path.exists(checkpoint_path):
        ...
        train_runner.agents['agent_1'].algo.actor_critic.load_state_dict(agent_state_dict['agent_1'])
        train_runner.agents['agent_1'].algo.optimizer.load_state_dict(optimizer_state_dict['agent_1'])
    ...
    for j in range(initial_update_count, num_updates):
        ...
        if log:
            ...
            if evaluator is not None and (j % args.test_interval == 0 or j == num_updates - 1):
                test_stats = evaluator.evaluate(train_runner.agents['agent_1'])
                stats.update(test_stats)
                if args.use_accel_paired:
                    adv_test_stats = evaluator.evaluate(train_runner.agents['adversary_agent_1'])
